...
export const trackingRoutes: FastifyPluginAsync = async (app) => {
  // Public — no auth. Returns only patient-safe data.
  app.get('/:token', async (req, reply) => {
    ...
    return {
      ...
      recipientName: stop.recipientName.split(' ')[0], // first name only for privacy
      stopsAhead,
      windowStart: stop.windowStart,
      windowEnd: stop.windowEnd,
      completedAt: stop.completedAt,
      // Share driver location only when very close (2 stops or less)
      driverLocation:
