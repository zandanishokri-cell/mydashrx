...
import { ArrowLeft, Plus, Zap, Send, Trash2, UserPlus } from 'lucide-react';
...
interface Stop { id: string; routeId: string; recipientName: string; address: string; recipientPhone: string; status: string; sequenceNumber: number | null; requiresRefrigeration: boolean; controlledSubstance: boolean; requiresSignature: boolean; requiresPhoto: boolean; rxNumbers: string[]; packageCount: number; deliveryNotes?: string; codAmount?: number; trackingToken?: string; arrivedAt?: string; completedAt?: string; failureReason?: string; failureNote?: string; }
...
export default function PlanDetailPage({ params }: { params: { planId: string } }) {
  ...
  const [showAddStop, setShowAddStop] = useState<string | null>(null); // routeId
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {stop.requiresRefrigeration && (
                              <span className="text-xs bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded">❄ Cold</span>
                            )}
                            {stop.controlledSubstance && (
                              <span className="text-xs bg-orange-50 text-orange-600 px-1.5 py-0.5 rounded">⚠ Ctrl</span>
                            )}
                          </div>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          {stop.rxNumbers?.length > 0 && (
                            <span className="text-xs text-gray-400">Rx ×{stop.rxNumbers.length}</span>
                          )}
